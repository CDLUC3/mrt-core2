...
public class GetProp
{
    ...
    public static long getNumLongEx(Properties prop, String key)
        throws TException
    {
        String retVal = prop.getProperty(key);
        if (StringUtil.isEmpty(retVal)) {
            throw new TException.INVALID_OR_MISSING_PARM(MESSAGE + "number missing:" + key);
        }
        long numVal = 0;
        try {
            numVal = Long.parseLong(retVal);
        } catch (Exception ex) {
            throw new TException.INVALID_OR_MISSING_PARM(MESSAGE + "number prop invalid:" + retVal);
        }
        return numVal;
    } 
