<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:param name="merrittBase">xxx</xsl:param>
    ...
    <xsl:template match="/">
        ...
        <xsl:variable name="rest">
            <xsl:for-each select="/*[1]">
                <xsl:value-of select="substring-after(namespace-uri(),'uc3.cdlib.org')" />
            </xsl:for-each>
        </xsl:variable>
                ...
                <table border="1">
                    ...
                    <xsl:for-each select="//*">
                        ...
                        <tr>
                            ...
                            <td>
                            <xsl:choose>
                                 <xsl:when test="$merrittBase = 'xxx'">
                                     <xsl:value-of select="local-name()"/>
                                 </xsl:when>
                                 <xsl:otherwise>
                                     <xsl:element name="a">
                                         <xsl:attribute name="href">
                                             <xsl:value-of select="concat($merrittBase,$rest,'#',local-name())"/>
                                         </xsl:attribute>
                                         <xsl:value-of select="local-name()"/>
                                         </xsl:element>
                                 </xsl:otherwise>
                                    ...
                                    <xsl:choose>
                                        <xsl:when test="contains($value, '/state')">
                                            <xsl:variable name="value2">
                                                <xsl:choose>
                                                    <xsl:when test="contains($value, '?')"><xsl:value-of select="concat($value, '&amp;t=xhtml')"/></xsl:when>
                                                    <xsl:otherwise><xsl:value-of select="concat($value, '?t=xhtml')"/></xsl:otherwise>
                                                </xsl:choose>
                                            </xsl:variable>
                                            <xsl:element name="a">
                                                <xsl:attribute name="href">
